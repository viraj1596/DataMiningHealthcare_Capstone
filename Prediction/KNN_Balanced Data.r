...
pos <- df_train[ which(df_train$readmited==1), ]
negative <- df_train[which(df_train$readmited==0),]
...
#knn
train_input     <- as.matrix(df_final[,-144])
train_output    <- as.vector(df_final[,144])
...
for (i in 1:kmax){
  ...
  CM1 <- table(prediction, df_final$readmited)
...
CM1 <- table(prediction, df_final$profit)
